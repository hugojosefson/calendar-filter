/** @module `/webcal` request parsing and validation. */

import { ApiError } from "./api-error.ts";
import type { ResolvedCalendarFilterOptions } from "../server/options.ts";
import { compileRule, type FilterRule } from "../filter/rules.ts";

/** Validated `/webcal` query state. */
...
/** Rejects oversized request URLs before parsing or upstream work. */
...
/** Parses the public query format and enforces its input limits. */
...
/** Enforces the configured rule count before compiling another rule. */
...
/** Rejects unsafe or oversized replacement calendar names. */
...
/** Reports controls forbidden in a calendar display name. */
...
/** Applies the source-byte limit before regular-expression compilation. */
...
/** Accepts only unique RE2 flags supported by the API. */
...
/** Normalizes webcal to HTTPS and rejects credential-bearing upstream URLs. */
